 ...
 * @returns {RequestBuilder}
 */
RequestBuilder.compile = RequestBuilder.fromJSON = function (req, config) {
  requestBuilder = new RequestBuilder(req);
...
RequestBuilder.prototype.invoke = function (parameters, cb) {
  ...
  if (this.config) {
    req.cert = this.config.clientCert;
    req.key = this.config.clientKey;
  }
