...
/**
 * Parse the request template to collect variable definitions
 * @returns {{}}
 */
RequestBuilder.prototype.parse = function() {
    if(this.schema) {
        return schema;
    }
    var schema = {};
    ...
    if (json.variables) {
        for (var v in json.variables) {
            if (schema.hasOwnProperty(v)) {
                ...
                for(var a in json.variables[v]) {
                    schema[v][a] = json.variables[v][a];
                }
            }
        }
    }
