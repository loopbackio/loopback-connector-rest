...
function defineFunctions() {
  ...
  spec.operations.forEach(function(op) {
    ...
    const builder = RequestBuilder.compile(op.template, requestFunc);
    ...
    if (functions) {
      for (const f in functions) {
        ...
        const fn = builder.operation(functions[f]);
        functions[f] = fn;
      }
    }
  });
