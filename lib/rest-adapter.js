...
exports.initialize = function initializeRest(schema, callback) {
    ...
    if (settings.operations) {
        settings.operations.forEach(function (op) {
            if (!op.request) {
                throw new Error('The operation request is missing: ', op);
            }
            var builder = RequestBuilder.compile(op.request);
            var params = op.parameters || op.parameterNames;
            if (params) {
                if(!op.name) {
                    throw new Error('The operation name is missing: ', op);
                }
                if(settings.debug) {
                    console.log('Mixing in method: ', op);
                }
                var fn = builder.operation(params);
                schema[op.name] = fn;
                ...
                schema.defineOperation(op.name, fn, fn);
            } else {
                var fn = function() {
                    return builder.invoke(arguments);
                }
                if(settings.debug) {
                    console.log('Mixing in method: ', fn);
                }
                schema['invoke'] = fn;
                fn.accepts = [{name: 'request', type: 'object'}];
                fn.shared = true;
                schema.defineOperation('invoke', fn, fn);
